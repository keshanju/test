 ...
 * @LastEditTime: 2019-09-07 09:37:49
...
import { ArticleSpiderApi } from "../../apis/ArticleApi";
import { ArticleSpiderModel } from '@/models/ArticleSpiderModel';
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "文章采集";
  private article_spider_api = new ArticleSpiderApi();

  private loading: boolean = false;

  created() {
    this.article_spider_api = new ArticleSpiderApi();
  }
   ...
   */
  async getListPager() {
    let d = await this.article_spider_api.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ArticleSpiderModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ArticleSpiderModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this.article_spider_api.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
