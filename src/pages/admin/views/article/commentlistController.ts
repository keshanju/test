 ...
 * @LastEditTime: 2019-09-07 09:38:10
...
import { ArticleCommentApi } from "../../apis/ArticleApi";
import { ArticleCommentModel } from "@/models/ArticleCommentModel";
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "文章评论";
  private article_comment_api = new ArticleCommentApi();
  private loading: boolean = false;

  created() {
    this.article_comment_api = new ArticleCommentApi();
  }
   ...
   */
  async getListPager() {
    let d = await this.article_comment_api.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ArticleCommentModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ArticleCommentModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this.article_comment_api.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
