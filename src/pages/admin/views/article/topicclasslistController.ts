 ...
 * @LastEditTime: 2019-09-20 14:35:35
...
import { ArticleTopicClassApi } from "../../apis/ArticleApi";
import { ArticleTopicClassModel } from "@/models/ArticleTopicClassModel";
...
export default class List extends BaseVue {
  ...
  //表单
  private form: any = {};
  private title = "文章专题分类";
  private class_list = [];
  private _ArticleTopicClassApi = new ArticleTopicClassApi();
  private loading: boolean = false;
  created() {
    this._ArticleTopicClassApi = new ArticleTopicClassApi();
  }

  async mounted() {
    await this.getListPager();
    let d = await this._ArticleTopicClassApi.getList();
   ...
   */
  async getListPager() {
    let d = await this._ArticleTopicClassApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ArticleTopicClassModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ArticleTopicClassModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._ArticleTopicClassApi.setDelete(row.classid);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
