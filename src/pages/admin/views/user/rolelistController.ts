 ...
 * @LastEditTime: 2019-09-07 09:45:24
...
import { UserRoleApi } from "../../apis/UserApi";
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "用户角色";
  private _UserRoleApi = new UserRoleApi();
  private loading: boolean = false;

  created() {
    this._UserRoleApi = new UserRoleApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._UserRoleApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: UserRoleModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: UserRoleModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d =  await this._UserRoleApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
