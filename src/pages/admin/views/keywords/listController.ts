 ...
 * @LastEditTime: 2019-09-07 09:42:06
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "关键词";
  private _KeywordsApi = new KeywordsApi();
  private loading: boolean = false;

  created() {
    this._KeywordsApi = new KeywordsApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._KeywordsApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: KeywordsModel) {
    this.dialog.show = true;
    let d = await this._KeywordsApi.getModel(row.id);
    this.form = d.data;
  }
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: KeywordsModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._KeywordsApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
  ...
  async onSubmitHandler(sender: any) {
    ...
    let d = await this._KeywordsApi.setAdd(this.form);
    if (d.code == 0) {
      this.getListPager();
      this.$message("操作成功!");
    } else {
