 ...
 * @LastEditTime: 2019-09-07 09:44:48
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "网站模板";
  private _TemplateApi = new TemplateApi();
  private loading: boolean = false;

  created() {
    this._TemplateApi = new TemplateApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._TemplateApi.getListPager(
      "",
...
* 数据编辑
*/
  async onRowEditHandler(row: TemplateModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: TemplateModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d =  await this._TemplateApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
