 ...
 * @LastEditTime: 2019-09-07 09:39:22
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "缓存管理";
  private _CacheApi = new CacheApi();
  private loading: boolean = false;

  created() {
    this._CacheApi = new CacheApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._CacheApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: CacheModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: CacheModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._CacheApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
