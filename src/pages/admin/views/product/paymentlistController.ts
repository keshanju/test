 ...
 * @LastEditTime: 2019-09-06 17:02:03
...
import { ProductPaymentModel } from "@/models/ProductModel";
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "支付管理";
  private _ProductPaymentApi = new ProductPaymentApi();
  private loading: boolean = false;
  created() {
    this._ProductPaymentApi = new ProductPaymentApi();  
  }
   ...
   */
  async getListPager() {
    let d = await this._ProductPaymentApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ProductPaymentModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ProductPaymentModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._ProductPaymentApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
