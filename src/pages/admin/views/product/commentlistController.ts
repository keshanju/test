 ...
 * @LastEditTime: 2019-09-06 17:00:25
...
import { ProductCommentModel } from '@/models/ProductModel';
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "评论管理";
  private _ProductCommentApi = new ProductCommentApi();
  private loading: boolean = false;
  created() {
    this._ProductCommentApi = new ProductCommentApi(); 
  }
   ...
   */
  async getListPager() {
    let d = await this._ProductCommentApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ProductCommentModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ProductCommentModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d =await this._ProductCommentApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
