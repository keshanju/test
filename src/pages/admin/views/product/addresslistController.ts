 ...
 * @LastEditTime: 2019-09-07 09:43:37
...
import { ProductAddressModel } from "@/models/ProductModel";
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "地址管理";
  private _ProductAddressApi = new ProductAddressApi();
  private loading: boolean = false;
  created() {
    this._ProductAddressApi = new ProductAddressApi();  
  }
   ...
   */
  async getListPager() {
    let d = await this._ProductAddressApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: ProductAddressModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ProductAddressModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d =  await this._ProductAddressApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
