...
export default class List extends BaseVue {
  ...
  private _ProductClassApi = new ProductClassApi();
  ...
  created() {
    this._ProductClassApi = new ProductClassApi();
  }
  ...
  async getListPager() {
    this.loading=true;
    let d = await this._ProductClassApi.getListPager(
      "",
  ...
  async onRowDeleteHandler(row: ProductClassModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      ...
      let d = await this._ProductClassApi.setDelete(row.classid);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
  ...
  public async submitForm() {
    ...
    this.loading = true;
    if (this.Modifyid) {
      data = await new ProductClassApi().setUpdate(Number(this.Modifyid), this.form);
    } else {
      data = await new ProductClassApi().setAdd(this.form);
    }
    this.loading = false;
    if (data.code == 0) {
      this.dialog.show = false;
...
*/
  ...
  */
  public onSearchForm(formSearch) {
    this.options.search = '';
    let con = this.class_list.find((ele) => {
      return ele.id == formSearch.type
    })
    let searchType = "classname__include__" + con.name;
