 ...
 * @LastEditTime: 2019-09-07 09:41:42
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "留言管理";
  private _GuestApi = new GuestApi();
  private loading: boolean = false;

  created() {
    this._GuestApi = new GuestApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._GuestApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: GuestModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: GuestModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._GuestApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
