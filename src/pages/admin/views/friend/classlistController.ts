 ...
 * @LastEditTime: 2019-09-20 14:41:53
...
import { FriendClassApi } from "../../apis/FriendApi";
import { FriendClassModel } from '@/models/FriendClassModel';
...
export default class List extends BaseVue {
  ...
  //表单
  private form: any = {};
  private title = "友情链接分类";
  private _FriendClassApi = new FriendClassApi();
  private loading: boolean = false;

  created() {
    this._FriendClassApi = new FriendClassApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._FriendClassApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: FriendClassModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: FriendClassModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._FriendClassApi.setDelete(row.classid);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
