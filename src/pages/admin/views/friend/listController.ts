 ...
 * @LastEditTime: 2019-09-07 09:41:21
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "友情链接";
  private _FriendApi = new FriendApi();
  private loading: boolean = false;

  created() {
    this._FriendApi = new FriendApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._FriendApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: FriendModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: FriendModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._FriendApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
