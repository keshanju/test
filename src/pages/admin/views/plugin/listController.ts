 ...
 * @LastEditTime: 2019-09-07 09:43:13
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "插件管理";
  private _PluginApi = new PluginApi();
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._PluginApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: PluginModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: PluginModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d =  await this._PluginApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
