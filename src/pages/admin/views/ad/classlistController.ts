 ...
 * @LastEditTime: 2019-09-20 14:44:16
...
import { AdClassApi } from "../../apis/AdApi";
import { AdClassModel } from "@/models/AdClassModel";
...
export default class List extends BaseVue {
  ...
  private title = "图片轮播分类";

  private _AdClassApi = new AdClassApi();
  private loading: boolean = false;

  created() {
    this._AdClassApi = new AdClassApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._AdClassApi.getListPager(
      "",
   ...
   * 数据编辑
   */
  async onRowEditHandler(row: AdClassModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: AdClassModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._AdClassApi.setDelete(row.classid);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
