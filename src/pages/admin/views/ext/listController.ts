 ...
 * @LastEditTime: 2019-09-07 09:40:21
...
export default class List extends BaseVue {
  ...
  private form: any = {};
  private title = "字段扩展";
  private _ExtApi = new ExtApi();
  private loading: boolean = false;

  created() {
    this._ExtApi = new ExtApi();
  }
  ...
  async getListPager() {
    this.loading = true;
    let d = await this._ExtApi.getListPager(
      "",
  ...
  * 数据编辑
  */
  async onRowEditHandler(row: ExtModel) {
   ...
   * 数据删除
   */
  async onRowDeleteHandler(row: ExtModel) {
    this.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      let d = await this._ExtApi.setDelete(row.id);
      if (d.code == 0) {
        this.$message({
          type: 'success',
          message: '删除成功!'
